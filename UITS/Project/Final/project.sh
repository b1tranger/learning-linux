...
# NPC Coordinates
# Bartender (shifted right, behind the long counter)
BARTENDER_X=38; BARTENDER_Y=2
# Sober Patron (sitting at a chair in front of the counter, left side)
PATRON_X=24; PATRON_Y=6
# Drunk Patron (sitting at a chair in front of the counter, center-left)
DRUNK_X=32; DRUNK_Y=6
...
draw_player() {
    ...
    if [[ $frame -eq $last_frame ]]; then
        ...
        [[ $((frame % 2)) -eq 0 ]] && tput cup $((y+3)) $x; echo " ╱╲ " || tput cup $((y+3)) $x; echo " ┘└ "
    fi
    tput sgr0
}
...
draw_patron() {
    ...
    tput cup $py $px;       echo "  ◉  "
    # Row 1: Face
    tput cup $((py+1)) $px; echo " (─) "
    # Row 2: Torso (seated, leaning on table)
    if [[ $((idle_frame % 24)) -lt 12 ]]; then
        tput cup $((py+2)) $px; echo " /█\\ "
    else
        tput cup $((py+2)) $px; echo " /█_▌"
    fi
    # Row 3: Seated legs
    tput cup $((py+3)) $px;  echo "  ╨╨ "
    tput sgr0
}
...
draw_drunk() {
    ...
    if [[ $((idle_frame % 10)) -lt 5 ]]; then
        # Leaning left
        tput cup $dy $((dx-1));     echo "  ◎  "
        tput cup $((dy+1)) $((dx-1)); echo " (≈) "
        tput cup $((dy+2)) $((dx-1)); echo "\\▓█/ "
        tput cup $((dy+3)) $((dx-1)); echo "  └┘ "
    else
        # Leaning right
        tput cup $dy $((dx+1));     echo "  ◎  "
        tput cup $((dy+1)) $((dx+1)); echo " (≈) "
...
clear_footprints() {
    for i in {0..3}; do tput cup $((y+i)) $x; echo "     "; done
}
